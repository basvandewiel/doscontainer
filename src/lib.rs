...
impl CHS {
    ...
    pub fn from_bytes(bytes: [u8; 3]) -> CHS {
        ...
        // Put all those bits together into a Vec that's 24 bits ong
        ...
        // The sectors byte is only 6 bits long, so pad it with zeroes and fill it to 8.
        ...
        // The cylinders value is 10 bits long but we have room for 16, so pad with zeroes.
        ...
        // Pull in the two bits stored in the sectors byte
        chs_cylinders.extend_from_bitslice(&chs_bits[8..=9]);
        // Finish off with the actual cylinders byte itself
