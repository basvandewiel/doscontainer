...
impl VBR {
    ...
    pub fn new(partition: &Partition) -> Self {
        VBR {
            ...
            sectors_per_cluster: VBR::set_sectors_per_cluster(partition.sector_count),
            ...
            sectors_per_fat: VBR::set_sectors_per_fat(&partition),
    ...
    fn set_sectors_per_cluster(partition_sector_count: u32) -> u8 {
        if partition_sector_count < 8400 {
            panic!("Less than 8400 sectors is an error condition for FAT16");
        } else if partition_sector_count < 32680 {
            return 2;
        } else if partition_sector_count < 262144 {
            return 4;
        } else if partition_sector_count < 524288 {
            return 8;
        } else if partition_sector_count < 1048576 {
            return 16;
        } else if partition_sector_count < 2097152 {
            return 32;
        } else if partition_sector_count < 4194304 {
            return 64;
        } else {
    ...
    fn set_sectors_per_fat(partition: &Partition) -> u16 {
        let root_dir_sectors = ((512 * 32) + (512 - 1)) / 512;
        let tmpval1 = partition.sector_count - (1 + root_dir_sectors);
        let tmpval2: u32 =
            (256 * u32::from(VBR::set_sectors_per_cluster(partition.sector_count))) + 2;
